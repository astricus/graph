...
function getNormalizedNodeCoordinates(
  ...
  if (
    config.node?.viewGenerator ||
    sourceNode?.viewGenerator ||
    targetNode?.viewGenerator
  ) {
    return { sourceCoords, targetCoords };
  }
  ...
  let strokeSize =
    strokeWidth *
    Math.min(config.link?.markerWidth || 0, config.link?.markerHeight || 0);
  ...
  switch (sourceSymbolType) {
    case CONST.SYMBOLS.CIRCLE: {
      ...
      x1 +=
        (sourceNodeSize + (config.directed ? strokeSize : 0)) *
        directionVector.x;
      y1 +=
        (sourceNodeSize + (config.directed ? strokeSize : 0)) *
        directionVector.y;
  ...
  switch (targetSymbolType) {
    case CONST.SYMBOLS.CIRCLE: {
      ...
      x2 -=
        (targetNodeSize + (config.directed ? strokeSize : 0)) *
        directionVector.x;
      y2 -=
        (targetNodeSize + (config.directed ? strokeSize : 0)) *
        directionVector.y;
